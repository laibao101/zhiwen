...
$.ajax({
	type:'post',
	url:'show_all_user.php',
	async:false,
	success:function (response) {
		var all=$.parseJSON(response);
		$.extend(true,all_user_img, all);
...
$(function(){
	...
	$(".push").click(function(){
		...
		if($("#question input[name=title]").val()!=''){
			...
			$.ajax({
				...
				success:function(){
					/*问题提交成功后修改侧边栏情况*/
					$(".push").parents(".row").find("textarea[name=comment]").val('');
							...
							if(comment.hasClass("hide")){
								...
								$.ajax({
									...
									success: function (response) {
										...
										var count= 0;
										/*函数用于写头像进去*/
										function show_comment_img(json) {
											/*将存在头像的用户的头像写进去*/
											for(var i=0;i<json.length; i++){
												for(var j=0; j<all_user_img.length;j++){
													if(json[i].user==all_user_img[j].user){
														json[i].url=all_user_img[j].img
													}
												}
										...
										/*存在评论就加载评论*/
										if(json.length!=0){
											show_comment_img(json);
											count=Number(json[0].count);
											$(json).each(function (index,value) {
												html2+='<div class="row">\
												<p><span class="pub_user">'+$(this)[0].user+'</span><span>发表于</span><time>'+$(this)[0].date+'</time></p>\
											<div class="media-left col-lg-2 col-md-2 col-xm-3 col-xs-3">\
												<a href="javascript:;" class="thumbnail">\
												<img src="uploads/user/'+$(this)[0].url+'" alt="" />\
												</a>\
												</div>\
												<div class=" col-lg-10 col-md-10 col-xm-9 col-xs-9">\
												<p>\
											...
											if($.cookie("user")){
												if(comment.find("textarea").val()!='' ){//登录了填写了评论内容，才能发送提交请求
													$.ajax({
														...
														success: function () {
															console.log(all_user_img);
															var url='';
															for(var i=0;i<all_user_img.length; i++){
																if(all_user_img[i].user==$.cookie("user")){
																	url=all_user_img[i].img;
																}
															}
															if(url==''){//不存在当前用户的头像,设置为默认头像
																url='1.jpg';
															}
															...
															$("#loadding span").html("提交成功").css("background-image","url(img/reg_succ.png)");
															setTimeout(function () {
																var comment=$(".glyphicon-comment").parents("li").parents(".media").find(".comment");
																comment.find("textarea").val('') ;
																$("#loadding").modal("hide");
																$("#loadding span").html("正在提交").css("background-image","url(img/loading.gif)");
	...
	function show_user_img() {
		if(all_user_img.length){/*有用户头像*/
			var existence=false;
			for(var i=0; i<all_user_img.length; i++){
				if(all_user_img[i].user==$.cookie("user")){
					$("#user_img").html($.cookie("user"));
					$("#user_img").css("background-image","url(uploads/user/"+all_user_img[i].img+")");
					$("#side_member").prev().html('<img src="uploads/user/'+all_user_img[i].img+'" style=""  class="img-responsive img-circle" alt="">');
					existence=true;
				}
			}
			if(!existence){//不存在当前用户的头像
				not_exit_user_img();
			}
		}else{//数据库中一个用户头像都没有
			not_exit_user_img()
		}
		function not_exit_user_img() {
